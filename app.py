...
def launched():
    ...
    if context.System.device.supportedInterfaces.Display is None:
        ...
        hint = "Was läuft heute in Mannheim im Kino?"
...
def events(datum, plz, ort, suche):
    ...
    try:
        ...
        if context.System.device.supportedInterfaces.Display is None:
            ...
            return question(speech).list_display_render(template="ListTemplate1", backButton="HIDDEN", title=title,
                                                        listItems=items)
    except:
        speech = "Es wurden keine Veranstaltungen gefunden."
        if context.System.device.supportedInterfaces.Display is None:
            return statement(speech)
        else:
            text = {"primaryText": {"type": "RichText", "text": "Es wurde keine Veranstaltung gefunden"}}
            return statement(speech).display_render(template="BodyTemplate6", text=text, background_image_url=background)
...
def yes():
    try:
        if session.attributes["state"] == "end_of_data":
            speech = "Das waren alle Einträge"
            if context.System.device.supportedInterfaces.Display is None:
                return statement(speech)
            else:
                text = {"primaryText": {"type": "RichText", "text": speech}}
        ...
        speech = "Est ist ein Fehler unterlaufen"
        if context.System.device.supportedInterfaces.Display is None:
            return statement(speech)
        else:
            text = {"primaryText": {"type": "RichText", "text": speech}}
...
def iterate(events):
    ...
    for index, event in enumerate(events):
        ...
        if index == 0:
            event_str += "{} <break time='1s'/> Beim Veranstalter {} in {} läuft {}".format(intro, event["veranstalter"], event["stadt"],
                                                                     event["name_titel"])
        elif index < 2:
            event_str += "{} <break time='1s'/> Beim Veranstalter {} in {} läuft {}".format(neutral_response, event["veranstalter"],
                                                                     event["stadt"], event["name_titel"])
        else:
            session.attributes["count"] += 3
            if session.attributes['state'] == "end_of_data":
                event_str += "{} <break time='1s'/> Beim Veranstalter {} in {} läuft {} <break time='0.5s'/> Werbung {} {}".format(
                    neutral_response, event["veranstalter"], event["stadt"],
                    event["name_titel"], event_werbung, outro)
                break

            else:
                event_str += "{} <break time='1s'/> Beim Veranstalter {} in {} läuft {} <break time='0.5s'/> Werbung {} {} Möchtest Du noch mehr hören?".format(neutral_response, event["veranstalter"], event["stadt"],
                                                                         event["name_titel"], event_werbung, outro)
                break
...
def get_werbung():
    werbungen = ["https://s3-eu-west-1.amazonaws.com/werbungmoma/werbung1_alexa.mp3", "https://s3-eu-west-1.amazonaws.com/werbungmoma/werbung2_alexa.mp3", "https://s3-eu-west-1.amazonaws.com/werbungmoma/werbung3_alexa.mp3"]
